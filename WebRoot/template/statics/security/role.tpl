...
<body onpageshow="resetPagination()">
    ...
    {literal}
        ...
        function createOne(){
        	var dlg = 'dlg';
        	//
			url = '/pas/RoleService/createWithPerms.do';
            //
        ...
        function updateOne(){
            ...
            if (row){
                ...
                getForm(dlg).form('load',row);
				url = '/pas/RoleService/updateWithPerms.do?user_id='+row.user_id;
            } else {
            	alert("请选择要修改的行！");
            }
        }
        ...
        function removeOne(){
            ...
            if (row){
                $.messager.confirm('Confirm','删除后数据不能恢复，确认删除么?',function(r){
                    if (r){
                        $.post('/pas/RoleService/removeWithPerms.do',row,function(result){
                    		if (result.ReturnCode == 0){
