...
<body onpageshow="resetPagination()">
    ...
    {literal}
        ...
        function updateOne(){
            ...
            var row = getDatagrid(dg).treegrid('getSelected');
            getForm(dlg).form('clear');
            if (row){
                getDialog(dlg).dialog('open').dialog('setTitle','修改用户信息');
                getForm(dlg).form('load',row);
				url = '/pas/MenuService/update.do?user_id='+row.user_id;
            } else {
            	alert("请选择要修改的行！");
            }
        }
            ...
            getForm(dlg).form('submit',{
                ...
                success: function(result){
                    ...
                    if (result.errorMsg){
                        ...
                        getDatagrid('dg').treegrid('reload');    // reload the role data
                    }
                }
            });
        }
        function removeOne(){
            ...
            if (row){
                $.messager.confirm('Confirm','删除后数据不能恢复，确认删除么?',function(r){
                    if (r){
                        $.post('/pas/MenuService/remove.do',row,function(result){
                            if (result.success){
                                $.messager.show({    // show error message
                                    title: '提示',
                                    msg: '数据删除成功！'
                                });
                                getDatagrid('dg').treegrid('reload');
                            } else {
                                $.messager.show({    // show error message
                                    title: 'Error',
                                    msg: result.errorMsg
