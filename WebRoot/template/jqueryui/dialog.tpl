    <div id="{$id}" class="easyui-dialog" style="width:650px; height:480px;padding:10px 20px"
		...
		{elseif $field.type eq "combobox"}
            <div class="fitem" id="div-{$id}-{$field.name}">
                <label>{$field.title}:</label>	
                <input id="{$field.name}" name="{$field.name}" style="cursor:hand" class="easyui-combobox" {if $field.required eq 'true'}required{/if}
			        data-options="   
		...
		{elseif $field.type eq "combotree"}
            <div class="fitem" id="div-{$id}-{$field.name}">
                <label>{$field.title}:</label>
                <input id="{$field.name}" name="{$field.name}" style="cursor:hand" class="easyui-combotree" {if $field.required eq 'true'}required{/if}
			        data-options="   
				        valueField: '{$field.valueField}',   
				        textField: '{$field.textField}', 
				        value:'{$field.value | default:''}',
    ...
    <div id="{$id}-buttons">
        <a href="javascript:void(0)" class="easyui-linkbutton c6" iconCls="icon-ok" onclick="{$action | default : 'saveOne'}('{$id}')" style="width:90px">保存</a>
